node {
    stage('Process Webhook and Post Status to GitHub') {
        ...
        def repoName = sh(script: 'echo "${PAYLOAD}" | jq -r ".repository.name"', returnStdout: false).trim()
        def branchName = sh(script: 'echo "${PAYLOAD}" | jq -r ".pull_request.head.ref"', returnStdout: false).trim()
        def commitSHA = sh(script: 'echo "${PAYLOAD}" | jq -r ".pull_request.head.sha"', returnStdout: false).trim()
        def gitAPIURL = sh(script: 'echo "${PAYLOAD}" | jq -r ".repository.url"', returnStdout: false).trim()
